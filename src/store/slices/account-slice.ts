...
import { TimeTokenContract, MemoTokenContract, MimTokenContract, wMemoTokenContract, FarmContract, StableReserveContract, RedemptionAbi, ARNSeedSale } from "../../abi";
...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.SEED_ADDRESS) {
        const seedContract = new ethers.Contract(addresses.SEED_ADDRESS, ARNSeedSale, provider);
        mySeedShare = await seedContract.userShare(address);
        stakeAllowance = 0; //await timeContract.allowance(address, addresses.STAKING_HELPER_ADDRESS);
    }
