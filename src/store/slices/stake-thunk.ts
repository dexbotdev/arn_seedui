...
import { StakingHelperContract, TimeTokenContract, MemoTokenContract, StakingContract, ARNSeedSale } from "../../abi";
...
export const changeStake = createAsyncThunk("stake/changeStake", async ({ action, value, provider, address, networkID }: IChangeStake, { dispatch }) => {
    ...
    const addresses = getAddresses(networkID);
    const signer = provider.getSigner();
    const staking = new ethers.Contract(addresses.SEED_ADDRESS, ARNSeedSale, signer);

    let stakeTx;
    const deposit = ethers.utils.parseEther(value);

    try {
        const gasPrice = await getGasPrice(provider);
        stakeTx = await staking.seedSale({ gasPrice: gasPrice, value: deposit });
